...
export default function GameBoard({ themeKey }) {
  ...
  const themes = {
    ...
    moana: { 
      name: "Moana Ocean Adventure", 
      light: "rgb(96, 255, 5)", 
      dark: "rgb(2, 97, 1)", 
      path: "/themes/moana/pieces/", 
      audioPath: "/themes/moana/sounds/" 
    }
  };
  ...
  async function onDrop(source, target) {
    ...
    try {
      ...
      setGame(gameCopy);
      // Playing specific capture sounds as requested
      if (move.captured) {
        playSound(turn === 'w' ? "white_capture.mp3" : "black_capture.mp3");
      } else {
        playSound("move.mp3");
      }
    ...
    return (
      <div style={{ minHeight: "100vh", backgroundColor: "#000", color: "white", padding: "40px", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: "40px", flexWrap: "wrap", marginTop: "30px" }}>
          <div style={{ width: "400px", padding: "30px", background: "#111", borderRadius: "15px", border: `4px solid ${currentTheme.light}` }}>
            <div style={{ display: "flex", gap: "10px", marginBottom: "20px" }}>
              <button onClick={() => setGameMode("ai")} style={{ flex: 1, padding: "10px", background: gameMode === "ai" ? currentTheme.light : "#333", color: "#000", border: "none", fontWeight: "bold" }}>VS AI</button>
              <button onClick={() => setGameMode("pvp")} style={{ flex: 1, padding: "10px", background: gameMode === "pvp" ? currentTheme.light : "#333", color: "#000", border: "none", fontWeight: "bold" }}>VS PLAYER</button>
            </div>
            <form onSubmit={handleStartGame} style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              <input placeholder="Your Username" value={inputs.p1} onChange={(e) => setInputs({...inputs, p1: e.target.value})} style={{ padding: "12px", borderRadius: "5px" }} required />
              ...
              <button type="submit" style={{ padding: "15px", background: currentTheme.light, color: "#000", fontWeight: "bold", border: "none", cursor: "pointer" }}>START GAME</button>
            </form>
          </div>
                ...
                {liveGames.map((g) => (
                  <tr key={g.id} style={{ borderBottom: "1px solid #222" }}>
                    ...
                    <td style={{ padding: "10px", textAlign: "right" }}>
                      <button onClick={() => handleStartGame(null, g)} style={{ padding: "5px 15px", background: "transparent", border: `1px solid ${currentTheme.light}`, color: currentTheme.light, cursor: "pointer" }}>JOIN</button>
                    </td>
                  </tr>
                ))}
              </tbody>
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#000", color: "white", textAlign: "center", padding: "40px" }}>
      ...
      <div style={{ width: "550px", margin: "0 auto", border: `12px solid ${currentTheme.dark}`, borderRadius: "8px" }}>
        <Chessboard 
            position={game.fen()} 
            onPieceDrop={onDrop} 
      ...
      <button onClick={() => window.location.reload()} style={{ marginTop: "30px", padding: "10px 30px", background: "#333", color: "#fff", border: "none", cursor: "pointer" }}>EXIT</button>
    </div>
  );
}
